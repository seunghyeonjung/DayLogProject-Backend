        ...
        return new Promise((resolve, reject)=>{
            ...
            db.query(query, [no, id] ,(err, data)=>{
                if(err) reject(err);
                    ...
                    //스크랩하기
                    const query13 = "UPDATE capstone_design.memberbadge SET goal_count = capstone_design.memberbadge.goal_count + 1 WHERE member_id=? and badge_no=? ";
                    db.query(query13, [id, 13]);
                    const query14 = "UPDATE capstone_design.memberbadge SET goal_count = capstone_design.memberbadge.goal_count + 1 WHERE member_id=? and badge_no=? ";
