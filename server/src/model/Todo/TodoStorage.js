...
class TodoStorage{
    static getTodo(id, where){
        return new Promise((resolve, reject)=>{
            //console.log(where);
            const query="SELECT *, DATE_FORMAT(todo_date,'%Y-%m-%d') AS todo_date, IF(todo_checked, 'true', 'false') as todo_checked FROM asas.todo " + where;
            //console.log(query);
    ...
    /*static getCheck(index){
        return new Promise((resolve, reject)=>{
            console.log(index);
            const query="SELECT *, DATE_FORMAT(todo_date,'%Y-%m-%d') AS todo_date, IF(todo_checked, 'true', 'false') as todo_checked FROM asas.todo WHERE todo_no=?";
    ...
    static saveTodo(id, content, date){
        return new Promise((resolve, reject)=>{
            ...
            const query="INSERT INTO asas.todo(member_id, todo_content, todo_date) VALUES(?, ?, ?)";
    ...
    static removeTodo(index){
        return new Promise((resolve, reject)=>{
            console.log(index);
            const query="DELETE FROM asas.todo WHERE todo_no=?";
            db.query(query, [index] ,(err, data)=>{
                if(err) reject(err);
    ...
    static modifyTodo(change, index){
        return new Promise((resolve, reject)=>{
            ...
            const query="UPDATE asas.todo SET todo_checked=? WHERE todo_no=?";
