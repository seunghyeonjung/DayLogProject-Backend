...
class ProfileStorage{
    static getProfile(id){
        return new Promise((resolve, reject)=>{
            ...
            const query="SELECT if(isnull(profile_src), null, CONCAT('http://localhost:3001/profiles/', profile_src)) as profile_src, member_id FROM asas.profile WHERE member_id=?";
            console.log(query);
    ...
    static saveProfile(id, profile){
        return new Promise((resolve, reject)=>{
            const query="INSERT INTO asas.profile(member_id, profile_src) VALUES(?, ?)";
    ...
    static modifyProfile(id, profile){
        return new Promise((resolve, reject)=>{
            const query="UPDATE asas.profile SET profile_src=? WHERE member_id=?";
