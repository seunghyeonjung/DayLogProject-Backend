...
class UserStorage{
    ...
    static getUserToken(id){
        return new Promise((resolve, reject)=>{
            const query="SELECT token FROM asas.tokens WHERE member_id=?";
            db.query(query, [id] ,(err, data)=>{
                if(err) reject(err);
                else resolve(data[0]);
            });
    ...
    static saveUserInfo(userInfo){
        return new Promise((resolve, reject)=>{
            const query="INSERT INTO asas.member(member_id, pw, nickname, name, email) VALUES(?, ?, ? , ?, ?)";
    ...
    static saveToken(id, token){
        return new Promise((resolve, reject)=>{
            const query="INSERT INTO asas.tokens(member_id, token) VALUES(?, ?)";
    ...
    static removeToken(id){
        return new Promise((resolve, reject)=>{
            const query="DELETE FROM asas.tokens WHERE member_id=?";
    ...
    static modifyUserInfo(id, pw, email, name, nickname){
        return new Promise((resolve, reject)=>{
            const query="UPDATE asas.member SET member_id=?, pw=?, email=?, name=?, nickname=? WHERE member_id=?";
